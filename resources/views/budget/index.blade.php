            ...
            <div class="card">
              <div class="card-header">
                <div class="row">
                  ...
                  <div class="col-md-6">
                    <button class="btn btn-primary pull-right ml-5" type="button" data-toggle="modal" data-target="#addLimit"><span><i class="mdi mdi-plus-circle-outline"></i></span>
                      Thêm hạn mức</button>
                  </div> 
                      ...
                      <table class="table display table-striped" id="datatable">
                          <tbody>
                            ...
                            @elseif($budget->spent <= $budget->budget )
                                  @foreach($cashlimit as $cash)
                                      @if( $budget->id == $cash->category)
                                      <tr>
                                        ...
                                        <td><strong>{{ number_format($budget->budget, 0, ',', '.') . ' ₫' }}</strong><br></span></td>
                                        @if(strtotime($cash->endday) >= time())
                                        <td>{{ date_format(date_create($cash->startday), 'd/m/Y') }} - {{ date_format(date_create($cash->endday), 'd/m/Y')  }}
                                        </td>
                                        @else
                                        <td>{{ date_format(date_create($cash->startday), 'd/m/Y') }} - {{ date_format(date_create($cash->endday), 'd/m/Y')  }}
                                        ...
                                        <td>
                                          <div>
                                              ...
                                              <strong class="pull-right">{{ number_format($budget->spent, 0, ',', '.') . ' ₫' }} / {{ number_format($budget->budget, 0, ',', '.') . ' ₫' }}</strong>
                                      ...
                                      <td>
                                        <div>
                                            ...
                                            <div class="progress progress-bar-danger-alt">
                                                <div class="progress-bar progress-bar-danger progress-bar-striped" role="progressbar" aria-valuenow="{{ $budget->percentage }}" aria-valuemin="0" aria-valuemax="100" style="width:{{ $budget->percentage }}%">
                                                </div>
                                            </div>
                                                      <div id="error-toast" class="toast bg-danger">
                                                          <div class="toast-body text-white text-center">
                                                            {{ __('budget.messages.over-spending') }}: {{ number_format($budget->spent - $budget->budget, 0, ',', '.') . ' ₫' }}
                                                          </div>
                                                      </div>
                                        </td>     
                                        <td>
                            ...
                            @endforeach
                            @else 
                            <tr class="text-center"><td colspans="6">Không có hạn mức chi nào ở đây</td></tr>
                            @endif
                          </tbody>
                      </table>
                  </div>
                    ...
                    <section class="text-center mt-15">
                      <svg class="circle-chart" viewbox="0 0 33.83098862 33.83098862" width="200" height="200" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <g class="circle-chart__info">
                          ...
                          <text class="circle-chart__subline" x="16.91549431" y="20.5" alignment-baseline="central" text-anchor="middle" font-size="2"> {{ number_format($stats['spent'], 0, ',', '.') . ' ₫' }} {{__('budget.info-box.spent')}}</text>
                        </g>
                      </svg>
                      <div class="chart-insights">
                        <p>{{__('budget.info-box.you-have-spent')}}</p>
                        <h4><strong>{{ number_format($stats['spent'], 0, ',', '.') . ' ₫' }}</strong> {{__('budget.info-box.out-of')}} <strong>{{ number_format($user->monthly_spending, 0, ',', '.') . ' ₫' }}</strong></h4>
                      </div>
                    </section>
                    <div></div>
              </div>
            ...
            @endif
              ...
              <div class="card-body">
                <div class="insight-card text-center">
                  ...
                  <p>{{ sprintf(__('budget.info-box.overbudget'), ( number_format($stats['spent'], 0, ',', '.') . ' ₫') , ( $stats['percentage'] - 100 )) }}</p>
                  @else
...
<div class="modal budget fade" id="create" role="dialog">
    <div class="modal-dialog modal-lg">
        ...
        <div class="modal-content">
            <form class="simcy-form" action="{{ route('budget.adjust') }}" data-parsley-validate="" loader="true" method="POST" enctype="multipart/form-data">
                @csrf
                              ...
                              <div class="form-group">
                                  <div class="row">
                                      <div class="col-md-6">
                                          ...
                                          <input type="number" class="form-control prefix" name="monthly_spending"  oninput="updateSpan()" id="monthly_spending" data-parsley-pattern="^[0-9]*\$" value="{{ $user->monthly_spending }}" placeholder="i.e 4000" required>
                                          <span class="help">{{__('budget.budget-form.label.per-month')}}</span>
                                      </div>
                                      <div class="col-md-6">
                                          <label>{{__('budget.budget-form.label.spend-annual')}}</label>
                                          <span class="input-prefix">VNĐ</span>
                                          <input type="number" class="form-control prefix" name="annual_spending"  data-parsley-pattern="^[0-9]*\$" value="{{ $user->annual_spending }}" placeholder="i.e 12000" id="annualspend" required>
                              ...
                              <div class="form-group">
                                  <div class="row">
                                      <div class="col-md-6">
                                          ...
                                          <span class="input-prefix">VNĐ</span>
                                          <input type="number" class="form-control prefix" name="monthly_saving"  data-parsley-pattern="^[0-9]*\$" value="{{ $user->monthly_saving }}" placeholder="i.e 4000" required>
                                          <span class="help">{{__('budget.budget-form.label.per-month')}}</span>
                                      </div>
                                      <div class="col-md-6">
                                          <label>{{__('budget.budget-form.label.earn-month')}}</label>
                                          <span class="input-prefix">VNĐ</span>
                                          <input type="number" class="form-control prefix" name="monthly_earning"  data-parsley-pattern="^[0-9]*\$" value="{{ $user->monthly_earning }}" placeholder="i.e 12000" required>
                                          <span class="help">{{__('budget.budget-form.label.per-month')}}</span>
                                      </div>
                                  </div>
                              </div>
                          ...
                          <div class="distribute">
                            ...
                            <div class="distribute-input">
                              <div class="form-group">
                                  <div class="row">
                                      <div class="col-md-12">
                                          <strong class="pull-right"><span class="allocated-budget">{{ number_format($category->budget, 0, ',', '.') . ' ₫' }} </span> / <span >{{ number_format($user->monthly_spending, 0, ',', '.') . ' ₫' }}</span></strong>  {{-- //class="total-budget" --}} 
  ...
  <div class="modal-dialog modal-sm">
      ...
      <div class="modal-content">
          ...
          <form class="simcy-form" action="{{ route('budget.add') }}" data-parsley-validate=""
              loader="true" method="POST" enctype="multipart/form-data" onsubmit="return kiemTraNgay()">
              @csrf
                  ...
                  <div class="form-group">
                      <div class="row">
                          <div class="col-md-12 ">
                              ...
                              <select class="form-control select2" name="category[]" id="categoryAdd" multiple="multiple">
                                   @if (!empty($categories)) 
                  ...
                  <div class="form-group">
                      <div class="row">
                          <div class="col-md-12">
                              ...
                              <input type="date" class="form-control datepicker" name="start_date" id="start"
                                  placeholder=""
                                  required="">
                          </div>
                      </div>
                  ...
                  <div class="form-group">
                      <div class="row">
                          <div class="col-md-12">
                              ...
                              <input type="date" class="form-control datepicker" name="end_date" id="end"
                                  placeholder="" >
              ...
              <div class="modal-footer">
                  ...
                  <button type="submit"
                      class="btn btn-primary">Lưu hạn mức</button>
              </div>
          </form>
      </div>
...
<!-- scripts -->
{{-- <script src="{{ route('') }}lang/{{env('APP_LOCALE_DEFAULT')}}/simcify-lang.js"></script> <!-- js language translation --> --}}
